...
<body>
                ...
                <div class="navbar-right">
                    ...
                    <div class="navbar-icons">
                        ...
                        <div class="user-profile-mini">
                                    <?php
                                        $sessNome = $_SESSION['usuario_nome'] ?? null;
                                        $sessNick = $_SESSION['usuario_nickname'] ?? null;
                                        $sessFoto = $_SESSION['usuario_foto'] ?? null;
                                        $displayNameMini = $sessNome ?: ($sessNick ?: 'Usuário');
                                        $avatarSrc = $sessFoto ? $sessFoto : 'images/avatar-default.svg';
                                    ?>
                                    <img src="<?php echo htmlspecialchars($avatarSrc); ?>" alt="Perfil" id="userAvatarMini">
                                    <div class="user-info-mini">
                                        <span id="userNameMini"><?php echo htmlspecialchars($displayNameMini); ?></span>
                                        <small id="userNicknameMini"><?php echo $sessNick ? '@'.htmlspecialchars($sessNick) : ''; ?></small>
                                    </div>
                                </div>
                        <script>
                            // Inline override temporário para exibir avatar/nome imediatamente
                            (function(){
                                try {
                                    var mock = { nome: 'Teste Usuário', nickname: 'testeuser', foto: 'images/avatar_drive_1.jpg' };
                                    window.usuarioAtual = window.usuarioAtual || mock;
                                    // Aplicar no DOM
                                    var nm = document.getElementById('userName');
                                    var nmMini = document.getElementById('userNameMini');
                                    var nkMini = document.getElementById('userNicknameMini');
                                    var av = document.getElementById('userAvatarMini');
                                    if (nm) nm.textContent = mock.nome;
                                    if (nmMini) nmMini.textContent = mock.nome;
                                    if (nkMini) nkMini.textContent = '@' + mock.nickname;
                                    if (av && av.tagName === 'IMG') { av.src = mock.foto; av.alt = mock.nome; av.onerror = function(){ this.onerror=null; this.src='images/avatar-default.svg'; }; }
                                } catch(e) { console.warn('inline mock failed', e); }
                            })();
                        </script>
                    </div>
                </div>
    ...
    <!-- Override temporário de usuário (apenas para desenvolvimento) -->
    <script src="session_user_override.js"></script>
